...
export class OpenOrdersContainer extends React.Component<Props> {
    ...
    public render() {
        ...
        return (
            <div className={classNames}>
                ...
                {fetching ? <div className="open-order-loading"><Spinner animation="border" variant="primary"/>
                </div> : this.openOrders()}
            </div>
        );
    }
    ...
    private renderHeaders = () => {
        const currentAskUnit = this.props.currentMarket ? (this.props.currentMarket.base_unit === 'rls') ? ' (تومان)' : ` (${this.props.currentMarket.base_unit.toUpperCase()})` : '';
        const currentBidUnit = this.props.currentMarket ? (this.props.currentMarket.quote_unit === 'rls') ? ' (تومان)' : ` (${this.props.currentMarket.quote_unit.toUpperCase()})` : '';
        ...
        return list.map((item: OrderCommon) => {
            ...
            return [
                ...
                <span style={{color: setTradeColor(side).color}}
                      key={id}>{preciseData(remainingAmount, amountFixed)}</span>,
