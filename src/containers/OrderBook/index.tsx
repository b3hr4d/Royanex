...
class OrderBookContainer extends React.Component<Props, State> {
    ...
    public render() {
        ...
        return (
            <div className={cn} ref={this.orderRef}>
                {orderBookLoading ?
                    <div className="pg-combined-order-book-loader"><Spinner animation="border" variant="primary"/>
                    </div> : this.orderBook(bids, asks)}
            </div>
        );
    }
    ...
    private lastPrice = () => {
        ...
        if (currentMarket && currentTicker) {
                ...
                <React.Fragment>
                    <span className={cn}>
                        {<NumberFormat displayType="text"
                                       value={Decimal.format(+(currentTicker.last), currentMarket.price_precision)}
                                       thousandSeparator={true}/>}&nbsp;
                        {isMobileDevice ? null : currentMarket.quote_unit === 'rls' ? 'تومان' : currentMarket.quote_unit.toUpperCase()}
                    </span>
            ...
            return <React.Fragment><span
                className={'cr-combined-order-book__market-negative'}>0</span><span>{this.props.intl.formatMessage({id: 'page.body.trade.orderbook.lastMarket'})}</span></React.Fragment>;
        }
    };
    ...
    private renderHeaders = () => {
        ...
        const formattedBaseUnit = (currentMarket && currentMarket.base_unit) ? (currentMarket.base_unit === 'rls') ? ' (تومان)' : `(${currentMarket.base_unit.toUpperCase()})` : '';
        const formattedQuoteUnit = (currentMarket && currentMarket.quote_unit) ? (currentMarket.quote_unit === 'rls') ? ' (تومان)' : `(${currentMarket.quote_unit.toUpperCase()})` : '';
        ...
        return (array.length > 0) ? array.map((item, i) => {
            ...
            switch (side) {
                case 'asks':
                    ...
                    if (isMobileDevice) {
                        return [
                            <span key={i}><Decimal fixed={priceFixed} thousSep={','} floatSep={'.'}
                                                   prevValue={array[i + 1] ? array[i + 1][0] : 0}>{price}</Decimal></span>,
                            <Decimal key={i} thousSep={','} floatSep={'.'}
                                     fixed={(currentMarket && currentMarket.quote_unit === 'rls' ? 0 : amountFixed)}>{Number(volume)}</Decimal>,
                        ];
                    }

                    return [
                        <span key={i}><Decimal thousSep={','} floatSep={'.'} fixed={priceFixed}
                                               prevValue={array[i + 1] ? array[i + 1][0] : 0}>{price}</Decimal></span>,
                        <Decimal key={i} thousSep={','} floatSep={'.'} fixed={amountFixed}>{volume}</Decimal>,
                        <Decimal key={i} thousSep={','} floatSep={'.'}
                                 fixed={(currentMarket && currentMarket.quote_unit === 'rls' ? 0 : amountFixed)}>{Number(volume) * Number(price)}</Decimal>,
                    ];
                default:
                    if (isLarge) {
                        if (isMobileDevice) {
                            return [
                                <Decimal key={i} fixed={(currentMarket && currentMarket.quote_unit === 'rls' ? 0 : amountFixed)} thousSep={','}
                                         floatSep={'.'}>{Number(volume)}</Decimal>,
                                <span key={i}><Decimal
                                    thousSep={','}
                                    floatSep={'.'}
                                    fixed={priceFixed}
                        ...
                        return [
                            <Decimal key={i} thousSep={','} floatSep={'.'}
                                     fixed={(currentMarket && currentMarket.quote_unit === 'rls' ? 0 : amountFixed)}
                            >{Number(volume) * Number(price)}</Decimal>,
                            <Decimal key={i} fixed={amountFixed} thousSep={','} floatSep={'.'}>{volume}</Decimal>,
                            <span key={i}><Decimal
                                thousSep={','}
                                floatSep={'.'}
                                fixed={priceFixed}
