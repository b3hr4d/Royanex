...
class OrdersComponent extends React.PureComponent<Props, OrdersState> {
    public componentDidMount() {
        ...
        this.props.userOrdersHistoryFetch({pageIndex: 0, type, limit: 10});
    }
    ...
    public renderContent = list => {
        const {firstElemIndex, lastElemIndex, pageIndex, nextPageExists, dashboard, title} = this.props;

        return (
            <React.Fragment>
                <History title={title} headers={this.renderHeaders()} data={this.retrieveData(list)}/>
                {!dashboard &&
                <Pagination
    ...
    private onClickPrevPage = () => {
        ...
        this.props.userOrdersHistoryFetch({pageIndex: Number(pageIndex) - 1, type, limit: 10});
    };
...
const mapStateToProps = (state: RootState): ReduxProps => ({
    ...
    firstElemIndex: selectOrdersFirstElemIndex(state, 10),
    list: selectOrdersHistory(state),
    fetching: selectOrdersHistoryLoading(state),
    lastElemIndex: selectOrdersLastElemIndex(state, 10),
