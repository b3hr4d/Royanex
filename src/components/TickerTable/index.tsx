            ...
            {props.currentBidUnitsList.map((item, i) => (
                ...
                >
                    <span className="navigation__item__link">
                        {item ? intl.formatMessage({id: `page.body.coins.${item}`}) : intl.formatMessage({id: 'page.body.marketsTable.filter.all'})}
                    </span>
                </li>
            ))}
        </ul>
            ...
            <tr key={index} onClick={() => props.redirectToTrading(market.id)}>
                <td>
                    <div>
                        ...
                        {market && (intl.formatMessage({id: `page.body.coins.${market.base_unit}`}) + ' / ' + intl.formatMessage({id: `page.body.coins.${market.quote_unit}`}))}
                    </div>
                </td>
                <td>
                    <span>
                        <Decimal
                            fixed={(market.quote_unit === 'rls' || market.base_unit === 'rls') ? 0 : market.amount_precision}
                            thousSep=",">
                            {market.last}
                        </Decimal>
                    </span>
            ...
            <div className="pg-ticker-table__table-wrap">
                <table className="pg-ticker-table__table">
                    ...
                    {markets[0] ? (
                        ...
                        <tr>
                            <td><span className="no-data">{intl.formatMessage({id: 'page.noDataToShow'})}</span></td>
                        </tr>
                    )}
                    </tbody>
                </table>
