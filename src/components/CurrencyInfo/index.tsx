                ...
                <div className="cr-wallet-item__single-balance">
                    <div>
                        <span className="cr-wallet-item__balance">
                            <FormattedMessage id="page.body.wallets.balance"/>&nbsp;{currency}
                        </span>
                        &nbsp;
                    ...
                    {currency === 'RLS' &&
                    <Link to={`/deposit/riyal`} className="btn btn-success mt-4">واریز</Link>
                    }
                    <Link to={`/withdraw/${currency.toLowerCase()}`} className="btn btn-danger mt-4">برداشت</Link>
                </div>
            </div>
            {props.wallet && props.wallet.type === 'coin' && props.address && true &&
            <div className="text-center">
                <EasyQRCode data={props.address} size={160} logo={findIcon(currency.toUpperCase())}/>
                <div className="alert alert-success text-center cr-wallet-item__copyaddress second-font" role="alert" onClick={handleCopyText}>
                    {props.address}
                </div>
            </div>
            }
